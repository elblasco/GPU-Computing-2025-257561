...
if [[ -z $SbM_HOME ]]; then
    ...
    if [[ ! -f SbatchMan/sourceFile.sh ]]; then
        ...
        SbatchMan/newExperiment.sh -p "edu-short" -t 00:04:00 -e "$EXPERIMENT_NAME_$(basename "${file%.*}")" -n 1 -c 1 -g 1 -d 1 -w edu02 -b "$BIN" # -a hackaton
    else
        source SbatchMan/sourceFile.sh
    fi
fi
...
for file in $MTX_PATH/*; do
    ...
    SbM_submit_function --verbose --expname "$EXPERIMENT_NAME_$(basename "${file%.*}")" --binary $BIN -f "$file" -n $ITERATIONS
    echo "JOB ID: ${job_id}"
done
