...
if [[ -z $SbM_HOME ]]; then
    ...
    SbatchMan/newExperiment.sh -p "edu-short" -t 00:05:00 -e "COO_SPMV" -n 1 -c 1 -g 1 -d 1 -w edu02 -b "$BIN"
fi
...
for file in "$MTX_PATH"/*.sbmtx; do
    ...
    SbM_submit_function --verbose --expname "$COO_SPMV_$(basename $file)" --binary $BIN -f "$file"
    echo "JOB ID: ${job_id}"
done
